...
先展示本 skill 创建或修改的文件，并询问用户是否要提交。只有用户明确同意后，才 stage 本 skill 改动的文件；不要 stage 工作树中无关的 changed/created files。建议 message：`Add pre-commit hooks (husky + lint-staged + prettier)`。

提交会经过新的 pre-commit hooks，可作为 smoke test。用户不想提交时，保留已验证的工作树改动并报告文件列表。
