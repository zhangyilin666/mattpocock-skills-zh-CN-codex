...
然后使用当前 harness 可用的探索型 sub-agent 并行遍历 codebase；在 Codex 中使用可用的 sub-agent / collaboration 机制。如果当前环境没有 sub-agent，就在主 agent 中分区域顺序探索。不要套死板 heuristics；自然探索，并记录你感到 friction 的地方：
