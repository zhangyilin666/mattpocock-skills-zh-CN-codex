...
- **Model-invoked** skill 使用包含明确触发条件的 frontmatter `description`，并让 `agents/openai.yaml` 中的 `policy.allow_implicit_invocation` 保持 `true` 或省略。它会带来 **context load**：description 进入可用技能索引，但用户仍可输入 `$skill-name` 显式调用。
- **User-invoked** skill 仍保留简洁、真实的 `description`，但在 `agents/openai.yaml` 中设置 `policy.allow_implicit_invocation: false`。Codex 不会默认把它注入模型上下文，用户仍可通过 `$skill-name` 调用；代价是用户需要记住它。

只有当 agent 必须自行找到该 skill，或另一个 skill 必须调用它时，才选择 model-invocation。如果它只会被手动触发，就做成 user-invoked，不付 context load。

当 user-invoked skills 多到用户记不住时，用一个 **router skill** 治理 cognitive load：它负责命名其他 user-invoked skills 以及何时使用它们。
