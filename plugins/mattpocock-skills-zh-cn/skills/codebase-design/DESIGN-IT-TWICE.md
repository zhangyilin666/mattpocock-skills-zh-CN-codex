...
使用当前 harness 提供的并行 sub-agent 能力启动 3+ 个独立探索任务。Codex 中使用可用的 sub-agent / collaboration 机制；其他 harness 使用其等价能力。如果当前环境没有 sub-agent，就在主 agent 中按相同约束依次生成方案。每个探索任务都必须为 deepened module 产出一个 **radically different** interface。
